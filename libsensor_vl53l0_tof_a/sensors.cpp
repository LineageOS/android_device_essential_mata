...
struct sensors_module_t HAL_MODULE_INFO_SYM = {
        common: {
                ...
                name: "STMicroelectronics VL53L0 sensor module",
...
int sensors_poll_context_t::activate(int handle, int enabled) {
	//Added for debug
	LOGE("sensors_poll_context_t::activate gets called!");
...
int sensors_poll_context_t::setDelay(int handle, int64_t ns) {
    //Added for debug
    LOGE("sensors_poll_context_t::setDelay gets called!");
...
int sensors_poll_context_t::pollEvents(sensors_event_t* data, int count)
{
	...
	//Added for debug
	//LOGE("sensors_poll_context_t::pollEvents gets called!");
...
{
    //Added for debug
    LOGE("sensors_poll_context_t::query gets called!");
    int index = handleToDriver(handle);
    if (index < 0) return index;
...
{
    //Added for debug
    LOGE("sensors_poll_context_t::batch gets called!");
    int index = handleToDriver(handle);
    if (index < 0) {
       LOGE("index: %d", index);
       return index;
                        ...
                        struct hw_device_t** device)
{
        ...
        dev->device.common.version  = SENSORS_DEVICE_API_VERSION_1_0; //TODO: Update to 1_3 on newer android environments
